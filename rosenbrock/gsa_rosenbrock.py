...
def run_gsa(show_logs=True, seed=None):
    N = 20; D = 2; max_iter = 1000; G0 = 100; alpha = 20; epsilon = 1e-7
    lower_bound = -2; upper_bound = 2; kbest_initial = N; kbest_final = 1
    if seed is not None: np.random.seed(seed)
    positions = lower_bound + (upper_bound - lower_bound) * np.random.rand(N, D)
    velocities = np.zeros((N, D)); best_agent_position = np.zeros(D)
    best_agent_fitness = float('inf'); positions_history = []; best_fitness_history = []
...
if __name__ == "__main__":
    ...
    output_dir = os.path.join(script_dir, "gsa_rosenbrock_output")
    os.makedirs(output_dir, exist_ok=True)
    print(f"Output files will be saved in '{output_dir}/'")

    max_iter_full = 1000; lower_bound = -2; upper_bound = 2
    x_plot = np.linspace(lower_bound, upper_bound, 400); y_plot = np.linspace(lower_bound, upper_bound, 400)
    X, Y = np.meshgrid(x_plot, y_plot); Z = fitness_function([X, Y])

    # --- FIGURE A: Convergence Graph ---
    fig_conv, ax_conv = plt.subplots(figsize=(10, 6))
    ax_conv.plot(np.arange(5, max_iter_full + 1), best_fitness_history[4:])
    ...
    ax_conv.set_yscale('log'); ax_conv.grid(True); plt.tight_layout()
    fig_conv.savefig(os.path.join(output_dir, 'convergence_1000iter.png'))

    # --- FIGURE B: Animation of First 20 Iterations ---
    fig_anim_20, ax_anim_20 = plt.subplots(figsize=(8, 7)); ax_anim_20.contourf(X, Y, Z, levels=np.logspace(0, 3.5, 35), cmap='viridis', alpha=0.7)
    ax_anim_20.set_title('GSA Agent Movement (First 20 Iterations)'); ax_anim_20.set_xlabel('x1'); ax_anim_20.set_ylabel('x2')
    ...
    iter_text_20 = ax_anim_20.text(0.02, 0.95, '', transform=ax_anim_20.transAxes, color='white', fontsize=12, bbox=dict(facecolor='black', alpha=0.5))
    ax_anim_20.legend()
    def update_anim_20(frame):
        scatter_20.set_offsets(positions_history[frame]); iter_text_20.set_text(f'Iteration: {frame + 1}/20')
    ...
    fig_anim_1000, ax_anim_1000 = plt.subplots(figsize=(8, 7)); ax_anim_1000.contourf(X, Y, Z, levels=np.logspace(0, 3.5, 35), cmap='viridis', alpha=0.7)
    ax_anim_1000.set_title(f'GSA Agent Movement (Full {max_iter_full} Iterations)'); ax_anim_1000.set_xlabel('x1'); ax_anim_1000.set_ylabel('x2')
    ...
    iter_text_1000 = ax_anim_1000.text(0.02, 0.95, '', transform=ax_anim_1000.transAxes, color='white', fontsize=12, bbox=dict(facecolor='black', alpha=0.5))
    ax_anim_1000.legend()
    def update_anim_1000(frame):
        scatter_1000.set_offsets(positions_history[frame]); iter_text_1000.set_text(f'Iteration: {frame + 1}/{max_iter_full}')
    ...
    fig_final_1000, ax_final_1000 = plt.subplots(figsize=(8, 7)); ax_final_1000.contourf(X, Y, Z, levels=np.logspace(0, 3.5, 35), cmap='viridis', alpha=0.7)
    ax_final_1000.set_title(f'Final Agent Positions at Iteration {max_iter_full}'); ax_final_1000.set_xlabel('x1'); ax_final_1000.set_ylabel('x2')
    ...
    ax_final_1000.plot(1, 1, 'y*', markersize=15, label='Global Minimum'); ax_final_1000.legend(); plt.tight_layout()
